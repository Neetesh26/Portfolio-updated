...
const Experience = () => {
  return (
    <section id="experience" className="relative py-28 px-6 max-w-7xl mx-auto min-h-screen flex flex-col justify-center">
      {/* Section Header */}
      <motion.div
      ...
      >
        <p className="text-[#14CF93] text-sm font-semibold tracking-widest uppercase">Career Journey & Technical Proficiency</p>
      ...
      <div className="space-y-8 max-w-5xl mx-auto w-full mb-20">
        <h3 className="text-2xl font-bold text-white tracking-wide flex items-center gap-3">
          <Briefcase className="w-6 h-6 text-[#14CF93]" />
        ...
        <div className="space-y-6">
          {workExperiences.map((exp, index) => (
            ...
            >
              <TiltCard className="p-8 space-y-4 border-l-4 border-l-[#14CF93]">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-white/10">
                  <div>
                    <span className="text-xs font-mono font-bold px-3 py-1 rounded-full bg-[#14CF93]/10 text-[#14CF93] uppercase tracking-wider">
                      {exp.type}
                    </span>
                    <h4 className="text-2xl font-extrabold text-white mt-2">{exp.role}</h4>
                    <div className="text-lg font-semibold text-[#14CF93] flex items-center gap-2 mt-0.5">
                  ...
                  <div className="flex items-center gap-2 text-xs font-mono text-gray-400 bg-white/5 px-4 py-2 rounded-xl border border-white/10 self-start sm:self-center">
                    <Calendar className="w-4 h-4 text-[#14CF93]" />
                ...
                <ul className="space-y-2.5 pt-2">
                  {exp.bulletPoints.map((point, idx) => (
                    <li key={idx} className="text-gray-300 text-sm leading-relaxed flex items-start gap-3">
                      <CheckCircle2 className="w-4 h-4 text-[#14CF93] shrink-0 mt-1" />
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
      ...
      <div className="space-y-8 max-w-5xl mx-auto w-full">
        <h3 className="text-2xl font-bold text-white tracking-wide flex items-center gap-3">
          <Code className="w-6 h-6 text-[#14CF93]" />
                ...
                <TiltCard className="p-6 space-y-4 h-full flex flex-col justify-between">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {cat.skills.map((skill) => (
                      <span
                        key={skill}
                        className="px-3 py-1.5 rounded-xl text-xs font-mono bg-white/[0.04] border border-white/10 text-gray-200 hover:border-[#14CF93]/50 hover:text-[#14CF93] transition-all"
                      >
                        {skill}
                      </span>
                    ))}
